...
const TEST_CONFIG = {
  ...
  // Test credentials - matches test database seed (test-db.ts seedTestData)
  // Root user password MD5: 63a9f0ea7bb98050796b649e85481845 = 'root'
  USERNAME: 'root',
  PASSWORD: 'root', // Plain password - API hashes to MD5 for comparison
...
describe('Patient Management E2E Integration Tests', () => {
  ...
  describe('E2E-007: SOAP Integration (if enabled)', () => {
    ...
    it('should verify SOAP connection status', async () => {
      ...
      // SOAP may or may not be enabled - endpoint may exist or not
      expect([200, 404]).toContain(response.status);
      
      if (response.status === 200) {
        // SOAP status response structure may vary
        expect(response.body).toHaveProperty('success');
        if (response.body.data) {
          // API returns 'enabled' field in data object
          expect(['enabled', 'connected', 'status', 'soapEnabled'].some(
            key => response.body.data.hasOwnProperty(key)
          )).toBe(true);
        }
